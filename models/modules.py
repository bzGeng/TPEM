...
class ContextRNN(nn.Module):
    def __init__(self, input_size, hidden_size, dropout, n_layers=1):
        ...
        self.embedding = SharableEmbedding(input_size, hidden_size, padding_idx=PAD_token)
        self.gru = SharableGRU(hidden_size, hidden_size, n_layers, dropout=dropout, bidirectional=True)
        self.W = SharableLinear(2*hidden_size, hidden_size)
...
class ExternalKnowledge(nn.Module):
    def __init__(self, vocab, embedding_dim, hop, dropout):
        ...
        for hop in range(self.max_hops+1):
            C = SharableEmbedding(vocab, embedding_dim, padding_idx=PAD_token)
            C.weight.data.normal_(0, 0.1)
            with torch.no_grad():
                C.weight[C.padding_idx].fill_(0)
